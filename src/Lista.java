...
public class Lista {
    ...
    /*public boolean eliminarMenorRendimento(){
        Jugador aux = obtenerMenorRendimiento();

        for (Jugador j : equipo){
            if(aux.getRendimiento() == j.getRendimiento()){
                return false;
            }
        }
    }*/
    ...
    /*public void ordenarAlfabeticamente() {
        ...
        for (int i = 0; i < equipo.size() - 1; i++) {
            for (int j = i + 1; j < equipo.size(); j++) {
                if (equipo.get(i).getNombre().compareToIgnoreCase(equipo.get(j).getNombre()) >= 1) {
                    aux = equipo.get(i);
                    equipo.set(i, equipo.get(j));
                    equipo.set(j,aux);
                }
